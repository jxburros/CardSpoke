...
**Key Finding:** Version 0.11.0 represents a **strategic pivot** from the planned Developer Ecosystem phase to implementing high-value Priority Features from the Post-1.0 roadmap section. While the v0.11 Developer Ecosystem deliverables (Extension Wizard, Playground, CardSpoke.utils) were not implemented, the project successfully delivered **7 major Priority Features** focused on knowledge management and user experience.
...
3. **CardSpoke.utils API** - Exposed helper library for mod developers
...
| ⚠️ partial | **CardSpoke.utils API** | app.js:1421, 1455, 2508+ | Functions exist (`createCard`, `updateCard`, `getTags`, `addTag`, `showToast`) but NOT exposed as `CardSpoke.utils` global API |
...
- ⚠️ Provide CardSpoke.utils helper library — **Functions exist, not exposed as API**
...
- ⚠️ CardSpoke.utils API — **Partially** (functions exist: `createCard`, `updateCard`, `addTag`, `getDatasetMeta`, `showToast` but NOT exposed as `CardSpoke.utils.*`)
...
3. **Expose CardSpoke.utils API** — Make utility functions available to mod developers as `window.CardSpoke.utils.*`
...
- CardSpoke.utils not exposed (planned feature, not debt)
...
**Next Update:** After v0.12.0 release or when v0.11.x patch addresses README and CardSpoke.utils API exposure.  
