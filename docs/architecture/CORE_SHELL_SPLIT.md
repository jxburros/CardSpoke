...
> **Note:** A legacy top-level `www/src/core.js` (a second, diverging copy of
> the plugin runtime built by an old `npm run build:cat` concatenation script)
> was **removed** — it was the source of the plugin-system instability. There
> is now one runtime (`www/src/core/`) and one build (`npm run build`, Vite).
> Do not reintroduce a second runtime copy or build path (see
> [`../PLUGIN_INVARIANTS.md`](../PLUGIN_INVARIANTS.md) §2).
