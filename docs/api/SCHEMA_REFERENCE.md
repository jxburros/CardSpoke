...
- Runtime constant `SCHEMA_VERSION` is defined in `www/src/01-metadata-and-utilities.js` (bundled into `www/app.js`) and surfaced to plugins via both `CardSpoke.utils.getDatasetMeta()` and the hook context.
