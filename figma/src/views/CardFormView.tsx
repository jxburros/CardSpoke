  ...
  return (
    <section className="max-w-3xl space-y-10">
      <header className="space-y-4">
        <h2 className="text-foreground text-[44px] font-semibold leading-[1.05]">{heading}</h2>
      ...
      <form className="space-y-8" onSubmit={handleSubmit}>
        <div className="space-y-2">
          <label className="text-xs uppercase tracking-[0.32em] text-muted-foreground">Title</label>
          <Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Card title" />
        </div>

        <div className="space-y-2">
          <label className="text-xs uppercase tracking-[0.32em] text-muted-foreground">Details</label>
          <Textarea
            value={body}
        ...
        <div className="space-y-2">
          <label className="text-xs uppercase tracking-[0.32em] text-muted-foreground">Parent Card</label>
          <select
        ...
        {editing && card && card.children.length > 0 && (
          <div className="space-y-5">
            <h3 className="text-xs uppercase tracking-[0.32em] text-muted-foreground">Existing Children</h3>
        ...
        <div className="space-y-5">
          <h3 className="text-xs uppercase tracking-[0.32em] text-muted-foreground">Add New Children</h3>
          <div className="space-y-4">
            {newChildren.map((value, index) => (
              <div key={index} className="flex gap-2">
                ...
                />
                <Button type="button" variant="ghost" onClick={() => setNewChildren((current) => current.filter((_, idx) => idx !== index))}>
                  Remove
                </Button>
              </div>
            ))}
          </div>
          <Button type="button" variant="outline" onClick={() => setNewChildren((current) => [...current, ""])}>
            + Add Another Child
          </Button>
        </div>
