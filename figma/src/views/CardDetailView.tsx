...
export function CardDetailView({
  ...
  return (
    <section className="space-y-14" ref={(el) => onCardRender(card.id, el)}>
      <header className="space-y-6">
        <h2 className="text-foreground text-[52px] font-semibold leading-[1.05]">{card.title || "(Untitled)"}</h2>
        {parent && (
        ...
        <article className="max-w-3xl whitespace-pre-wrap text-[18px] leading-relaxed text-muted-foreground">
      ...
      <div className="grid gap-10 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <div className="space-y-5">
          <h3 className="text-xs uppercase tracking-[0.32em] text-muted-foreground">Actions</h3>
        ...
        <div className="space-y-5 rounded-[32px] bg-card px-8 py-8 [box-shadow:var(--shadow-soft)]">
          <h3 className="text-xs uppercase tracking-[0.32em] text-muted-foreground">Metadata</h3>
      ...
      <section className="space-y-6">
        ...
        {children.length === 0 ? (
          <p className="text-muted-foreground/80">No children yet.</p>
        ) : (
          <div className="grid gap-6 md:grid-cols-2">
            {children.map((child) => (
              <button
                key={child!.id}
                className="rounded-[30px] bg-card px-8 py-8 text-left text-foreground transition-all duration-500 [box-shadow:var(--shadow-soft)] hover:-translate-y-1 hover:[box-shadow:var(--shadow-strong)]"
                onClick={() => onOpenCard(child!.id)}
                ref={(el) => onCardRender(child!.id, el)}
              >
                <div className="text-lg font-medium text-foreground">{child!.title || "(Untitled)"}</div>
                <div className="mt-3 text-sm text-muted-foreground">{bodyPreview(child!.body, 120)}</div>
              </button>
            ))}
          </div>
        )}
