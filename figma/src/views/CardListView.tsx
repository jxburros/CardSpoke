...
export function CardListView({
  ...
  const headingLabel = parentId ? "Nested Collection" : "Primary Collection";
    ...
    <section className="space-y-16">
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div className="space-y-3">
          <p className="text-xs uppercase tracking-[0.32em] text-muted-foreground/70">{headingLabel}</p>
          <h2 className="text-foreground text-4xl font-semibold md:text-5xl">{heading}</h2>
          <p className="max-w-xl text-base text-muted-foreground">
            {parentId
              ? `Showing ${sortedCards.length} cards inside this topic.`
              : "Surface high-level knowledge and drill down into detailed cards."}
          </p>
        </div>
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          <span>Sort</span>
          <select
            className="rounded-full border-0 bg-foreground/[0.05] px-5 py-2 text-sm text-foreground transition focus:bg-foreground/[0.1] focus:outline-none"
            value={sort}
      ...
      <div className="space-y-8">
        ...
        {sortedCards.length === 0 && (
          <div className="rounded-[40px] bg-foreground/[0.04] px-10 py-16 text-center text-muted-foreground">
      ...
      <div className="pt-4">
        <Button
          onClick={() => onAddCardHere(parentId)}
          size="lg"
          className="h-12 rounded-full bg-foreground text-background px-8 text-sm font-medium hover:bg-foreground/90"
        >
          + Add Card Here
        </Button>
      </div>
